...
- **LFN 支持**：所有文件操作支持 FAT 长文件名（如 `config.json` 小写），自动判断短名/LFN 并调用对应 CH378 API
...
config get <key>                           获取 Core 指定配置项的值
config get <module>                        获取指定模块的全部配置
config get <module> <key>                  获取指定模块中某配置项的值
config get <file.json>                     获取指定数据文件的全部配置
config get <file.json> <key>               获取指定数据文件中某配置项的值
config getkey                              列出 Core 所有键名
config getkey <module>                     列出指定模块所有键名
config getkey <file.json>                  列出指定数据文件所有键名
...
config rollback                            从 config.bak 回滚配置
...
获取 Core 指定配置项的值，仅输出原始值（不带 key: 前缀）：
...
获取指定模块中某配置项的值，仅输出原始值：

```
> config get 0101 brightness
80

> config get 0101 unknown_key
Error: key not found
```
...
从 `config.bak` 恢复配置（回滚），读取备份文件内容替换当前配置并保存应用：
...
> config rollback
Error: rollback failed (status=FF)
```

若 `config.bak` 不存在或解析失败，输出错误信息。

#### `config getkey`

列出 Core 所有键名（基于实际 JSON 数据，非预设表）：
...
CH378 文件管理芯片支持 FAT 长文件名（LFN），允许使用小写字母和超过 8.3 格式的文件名（如 `config.json`）。Config 模块内部实现了 LFN 自动适配：
...
- 全大写字母、数字、`_`、`~`、`-`，主名≤8字符，扩展名≤3字符 → 短文件名
- 包含小写字母或超长 → 长文件名，需使用 LFN API

### 12.2 LFN 辅助函数

Config 模块内部封装了三个 LFN 辅助函数，自动根据文件名类型选择 CH378 API：

```c
static uint8_t Config_LFN_Open(const char *path);    /* 打开文件 */
static uint8_t Config_LFN_Create(const char *path);  /* 创建/截断文件 */
static uint8_t Config_LFN_Erase(const char *path);   /* 删除文件 */
```

内部流程：
1. 从路径中提取文件名（最后一个 `\` 之后的部分）
2. 调用 `Config_IsShortName()` 判断
3. 短文件名 → 直接调用 `CH378FileOpen`/`CH378FileCreate`/`CH378FileErase`
4. 长文件名 → 调用 `Config_AsciiToUnicode()` 转换为 UTF-16LE，再调用 `CH378_Open_Long_Name`/`CH378_Create_Long_File`/`CH378_Erase_Long_Name`

### 12.3 文件写入流程

`Config_WriteBufToFile` 采用打开-截断-写入模式（与 CH378 驱动 `CH378_File_Write` 一致）：

1. `Config_LFN_Open(path)` — 尝试打开已有文件
   - 成功 → `CH378SetFileSize(0)` + `CH378ByteLocate(0)` 截断文件
   - `ERR_MISS_FILE` → `Config_LFN_Create(path)` 创建新文件
   - 其他错误 → 直接返回
2. `CH378ByteWrite()` — 循环写入数据
3. `CH378FileClose(1)` — 关闭并更新文件长度

> **注意**：不使用 create→close→open 序列，因为 `CH378_Create_Long_File` 对已有 LFN 文件的处理可能导致 CH378 进入异常状态。采用与 CH378 驱动内部一致的 open→truncate→write 方式更可靠。
