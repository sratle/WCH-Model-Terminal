...
Keyboard 模块有三种物理形态，均通过 UART3 与 Core（V5F）通信：

| 子类型 | 名称 | 主要特征 |
|--------|------|---------|
| `0x01` | Keyboard-1（主键盘） | 40 配列，CH9329 串口转 HID，标准 USB HID 键盘输出 |
| `0x02` | Keyboard-2（游戏键盘） | 2 摇杆 + 6 按钮 + 3 钮子开关 + 2 旋转编码器，CH9329 串口转 HID（键盘+鼠标混合模式） |
| `0x03` | Keyboard-3（音乐键盘） | 24 触摸琴键（TTP229） + 3 推子 + 12 按钮，无 CH9329，直连 Core |

### 1.1 帧格式速查

Keyboard 模块通过 **UART3** 与 Core（V5F）通信，物理层参数：
...
Keyboard-2（游戏键盘）上报 2 个摇杆、6 个按钮、3 个钮子开关、2 个旋转编码器的完整状态。

**HID 映射方案（CH9329 混合模式，实际固件实现）**：

| 输入设备 | CH9329 HID 映射 | 说明 |
|----------|-----------------|------|
| ROC1 方向 | 方向键（Up/Down/Left/Right） | 带迟滞：压下 <100/>200，释放 <120/>180；X/Y 轴硬件互换已在固件修正 |
| ROC2 X/Y | 鼠标相对移动 | (值-128)/8 缩放，保留模拟精度 |
| BUT1 | 鼠标左键 | mouse button bit0 |
| BUT2 | 鼠标右键 | mouse button bit1 |
| BUT3~BUT6 | H/J/K/L 键盘键 | |
| SW1 | Left Shift | 修饰键 bit1（上拨=按下） |
| SW2 | Left Ctrl | 修饰键 bit0（上拨=按下） |
| SW3 | Left Alt | 修饰键 bit2（上拨=按下） |
| EC1 旋转 | 鼠标 Y 轴偏移 | 增量 ×3 叠加 |
| EC2 旋转 | 鼠标 X 轴偏移 | 增量 ×3 叠加 |
| EC1_D | 鼠标中键 | button bit2 |
| EC2_D | 未映射 | 预留 |

> 钮子开关映射为修饰键：SW UP 位置 = 修饰键按下，SW 中间/DOWN = 修饰键释放。
...
DATA[4]: 按钮位图（bitmask）
  ...
  - bit4: BUT5    - bit5: BUT6
  - bit6~7: 预留
DATA[5]: 钮子开关状态（bitmask）
  - bit0~1: SW1 状态（00=中间, 01=UP, 10=DOWN, 11=保留）
  - bit2~3: SW2 状态（00=中间, 01=UP, 10=DOWN, 11=保留）
  - bit4~5: SW3 状态（00=中间, 01=UP, 10=DOWN, 11=保留）
  - bit6~7: 预留
DATA[6]: EC1 旋转增量（int8，正=顺时针，负=逆时针，读后清零）
DATA[7]: EC2 旋转增量（int8，正=顺时针，负=逆时针，读后清零）
DATA[8]: 编码器按下位图（bitmask）
  - bit0: EC1_D（编码器1按下）
  - bit1: EC2_D（编码器2按下）
  - bit2~7: 预留
```
...
> **身份 ACK 指纹约束**：Core 按 `LEN == 6 且 DATA[0] == 0x04` 识别 GET_TYPE 身份 ACK
> 并刷新槽位在线状态与 `type_id`。因此：
> 1. 身份 ACK 数据域**固定 5 字节**，不支持扩展字节（原"DATA[5..N] 扩展信息"已废弃）；
> 2. 查询类 ACK 数据域长度**不得为 5 字节**（现有查询均满足：背光 2B、琴键位图 2B、
>    按钮位图 1B、推子 6B、事件控制 1B、游戏输入 9B），新增查询命令时必须遵守。
...
| V1.2 | 2026-07-06 | Keyboard-2 硬件对齐：2 摇杆 + 6 按钮 + 3 钮子开关 + 2 旋转编码器；扩展 GAME_INPUT(0x29) 数据格式至 9 字节；定义 CH9329 键盘+鼠标混合模式 HID 映射 |
| V1.3 | 2026-07-27 | 热插拔可靠性加固：身份 ACK 指纹约束（LEN 固定 6，废弃扩展字节，查询 ACK 数据域禁止 5 字节）——修复背光/推子等查询 ACK 被 Core 误认身份帧污染 type_id 的缺陷；键盘失联时 Core 复位 type_id；模块侧 USART ISR 增加 ORE 清除 |
| V1.4 | 2026-08-08 | 文档对齐实际固件：Keyboard-2 HID 映射修正为实际实现（ROC1→方向键带迟滞、ROC2→鼠标移动 ÷8、EC 增量 ×3、EC1_D→鼠标中键、EC2_D 未映射、X/Y 轴硬件互换说明）；Keyboard-1 补充 74HC165 级联 39 键方案与 HID 报告节流说明（模块文档侧） |
