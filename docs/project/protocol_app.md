...
| 名称 | UUID | 方向 | 属性 |
|------|------|------|------|
| CLI Service | `0xFFE0` | — | Primary Service |
| CLI_RX | `0xFFF1` | APP → CH585F | Write / Write Without Response |
| CLI_TX | `0xFFF2` | CH585F → APP | Notify |

> **兼容性说明**：若希望兼容通用 BLE 串口调试工具（如 nRF Connect、LightBlue），可使用 Nordic UART Service (`0xFFE0`)，RX=`0xFFE1`，TX=`0xFFE2`。当前固件实际使用 Service `0xFFE0`、RX=`0xFFF1`、TX=`0xFFF2`，与标准 Nordic UART 的 RX/TX UUID 不同，但应用层帧格式与 UUID 选择无关。
...
> **实现状态**：当前 CH585F 固件仅处理 `MSG_TYPE_CLI_DATA`（0x01）与
> `MSG_TYPE_HEARTBEAT`（0xFF），收到 CTRL 帧直接忽略（不应答）。
> 本节控制命令为协议预留，待后续固件启用。
...
- 建议 APP 每 **5 秒** 发送一次心跳
- CH585F 收到后在本地立即回应（同 SEQ 回显，不转发 Core）
...
| V1.1 | 2026-08-08 | 对齐实际固件：CTRL 通道标注为预留（当前固件忽略）；心跳注明由 CH585F 本地同 SEQ 应答 |
