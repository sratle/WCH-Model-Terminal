...
本模块为独立蓝牙 / 无线 MCU，负责核心底板与上位机（手机 / PC）之间的无线连接，同时承担 OTA 升级和 WiFi 联网功能。
...
- **CH585F**：独立 RISC-V MCU，内置蓝牙协议栈（BLE / Classic）及 WiFi 基带
...
- 初始化 CH585F 自身时钟、GPIO、蓝牙协议栈、WiFi 协议栈
- 初始化 SPI4 从机模式，准备接收来自 V5F 的协议帧
- 设置默认广播名称、配对码等蓝牙参数
- 从 Flash 读取持久化配置（如已配对设备列表、WiFi 凭据）
...
- **WiFi 功能**：响应 V5F 的扫描 / 连接 / 断开指令，上报网络状态
...
4. WiFi 与蓝牙共用射频时，需注意协议栈的共存策略与天线复用逻辑。
5. 建议维护一个轻量级命令队列，避免 SPI 中断过于频繁导致蓝牙协议栈时序抖动。
