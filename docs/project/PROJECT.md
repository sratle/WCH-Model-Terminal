...
- Eink显示模块Display-2, 主控为CH32V307RCT6，通过SPI驱动墨水屏，墨水屏支持局部刷新。
  使用是648\*480的不支持触摸黑白墨水屏，但是实现了4*8分辨率的触摸板供用户操作。
...
> **⚠️ 历史设计说明**：本节描述的是早期 V3F+V5F 双核协作架构。当前固件已演进为
> **V5F 单核统一管理**（见第 3 节架构说明与 `Core.md` 架构演进章节）：所有模块 UART、
> 心跳槽位（`hardware_g.hb_slots`）、按键与配置均由 V5F 独占处理，V3F 仅负责上电唤醒
> V5F 后进入 STOP。以下内容仅作历史参考，不再反映现行实现。
