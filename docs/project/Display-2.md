...
Display-2 与 Display-1 一样接收 Core 经 `CMD_DISP_INPUT_EVENT`（0x15）转发的标准
HID 输入（CH9350 USB 键鼠 / Keyboard 模块 / BLE HID），并经 `CMD_DISP_EXT_HID_STATUS`
同步外接设备连接状态：

- **鼠标**：驱动与触摸板共享的同一光标（加速度曲线与 Display-1 一致），光标移动刷新
  同样受 350ms 光标节流合并；左键按下/抬起注入触摸事件（经手势合成产生
  CLICK/DOUBLE_CLICK，按住移动产生拖拽）；右键点击产生携带 `UI_MOUSE_BTN_RIGHT` 的
  CLICK 事件（文件应用弹出上下文菜单，同 Display-1）；滚轮增量先累计，每个 UI tick
  合并为一次 `scroll_delta` 事件投递，避免快速滚动时逐格触发约 300ms 的局部刷新。
...
| 长按/滑动 | 触控板模式不产生 | LONG_PRESS 保留给鼠标/绝对触摸路径；SWIPE 由 Core 侧键盘(PageUp/Down)/BLE 注入，外接鼠标拖拽也会经手势合成产生 |
...
- **输入**：触摸板/外接鼠标/键盘/Core 按键统一事件；触摸板与鼠标共享同一光标；手势合成 CLICK/DOUBLE_CLICK(300ms/20px)/LONG_PRESS(500ms)/SWIPE(60px)；HID 键盘全字符映射（含 Shift 与标点）+ 按住自动重复；方向键/翻页键/回车/Esc 直达页面事件，未消费时广播到控件；鼠标滚轮按 tick 合并投递（`scroll_delta`）
...
| 应用 | 状态 | 说明 |
|------|------|------|
| Music | ✅ 完整移植 | 播放/暂停/上下曲/播放列表/音量/扬声器开关，进度条为滑条控件（点按/拖动松手后发 `playloc` 拖进度），`playst` 同步 |
...
| 触摸矩阵 | ✅ 已完成 | 相对位移 + 光标节流（350ms，触摸板/鼠标共用） |
| 键盘字符输入 | ✅ 已完成 | HID 全字符映射 + 按键 diff + 按住自动重复 |
| 鼠标输入 | ✅ 已完成 | 共享光标/左右键/拖拽/滚轮（滚轮按 tick 合并投递） |
