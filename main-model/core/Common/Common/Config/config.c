...
void Config_Process(void)
{
    ...
    if (!hardware_g.config_req.pending) return;

    req = (config_request_t *)&hardware_g.config_req;
    req->pending = 0;

    /* 写操作需要设备匹配 */
    if (!Config_IsDeviceMatch() &&
        (req->type == CONFIG_REQ_SET_INT ||
         req->type == CONFIG_REQ_SET_STRING ||
         req->type == CONFIG_REQ_SAVE ||
         req->type == CONFIG_REQ_RESET)) {
        return;
    }

    switch (req->type) {
    case CONFIG_REQ_SET_INT:
        Config_SetInt(req->module_key, req->key, req->int_val);
        break;
    case CONFIG_REQ_SET_STRING:
        Config_SetString(req->module_key, req->key, req->str_val);
        break;
    case CONFIG_REQ_SAVE:
        Config_Save();
        break;
    case CONFIG_REQ_RESET:
        Config_ResetDefaults();
        break;
    case CONFIG_REQ_APPLY:
        Config_Apply();
        break;
    default:
        break;
