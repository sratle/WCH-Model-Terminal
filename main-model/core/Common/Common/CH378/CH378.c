...
static char ch378_current_path[CH378_MAX_PATH_LEN] = "\\";

/* 辅助：拼接基础路径和名称，生成绝对路径 */
static void CH378_Path_Join(const char *base, const char *name, char *out, uint16_t out_len)
{
...
static void CLI_Cmd_Ls(ch378_t *ch378)
{
    printf("--- %s ---\r\n", CH378_Dir_Get_Path());
    CH378_Dir_List(ch378, CLI_LsCallback);
}

static void CLI_Cmd_Cd(ch378_t *ch378, uint8_t argc, char **argv)
{
    uint8_t status;
    const char *dir;
    ...
    dir = argv[1];
    if (strcmp(dir, "..") == 0) {
        status = CH378_Dir_Go_Parent(ch378);
    } else if (strcmp(dir, "/") == 0 || strcmp(dir, "\\") == 0) {
        status = CH378_Dir_Go_Root(ch378);
    } else {
...
static void CLI_Cmd_Touch(ch378_t *ch378, uint8_t argc, char **argv)
{
    uint8_t status;
    ...
    status = CH378_File_Create(ch378, argv[1]);
    if (status == ERR_SUCCESS) {
...
static void CLI_Cmd_Cat(ch378_t *ch378, uint8_t argc, char **argv)
{
    char full_path[CH378_MAX_PATH_LEN];
    uint8_t status;
    uint8_t buf[256];
...
static void CLI_Cmd_Echo(ch378_t *ch378, uint8_t argc, char **argv)
{
    uint8_t i;
    ...
    if (redirect_idx > 0 && redirect_idx + 1 < argc) {
        ...
        status = CH378_File_Write(ch378, argv[redirect_idx + 1], (const uint8_t*)text_buf, pos);
        if (status == ERR_SUCCESS) {
...
void CH378_CLI(ch378_t *ch378, uint8_t *cmd, uint8_t len)
{
    ...
    if (strcmp(argv[0], "ls") == 0) {
        ...
        CLI_Cmd_Touch(ch378, argc, argv);
    } else if (strcmp(argv[0], "cat") == 0) {
        CLI_Cmd_Cat(ch378, argc, argv);
    } else if (strcmp(argv[0], "echo") == 0) {
        CLI_Cmd_Echo(ch378, argc, argv);
    } else if (strcmp(argv[0], "rm") == 0) {
        CLI_Cmd_Rm(ch378, argc, argv);
    } else {
