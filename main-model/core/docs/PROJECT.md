...
- CH585F是一个独立主控wireless-model，但是主要是作为一个蓝牙芯片来使用，
  ...
  蓝牙工作在主机模式，支持连接蓝牙键盘、蓝牙鼠标，同样可以触发HID事件通过UART4报告给Display模块。
  蓝牙支持连接蓝牙耳机，可以通过CH378获取音频之后用SPI传输给CH585F，CH585F接收之后转发给蓝牙耳机实现播放功能。有记忆功能，可以配对蓝牙设备后记住配对过哪些。
  有个移动端APP，可以通过蓝牙连接CH585F后输入命令控制设备，相当于一个蓝牙SSH CLI。
...
- 供电模块Power连接了一个6000mAh的电池，并且可以读取电量，可以单独拿出来作为充电宝使用，
