...
| `touch <file>` | 创建空文件 | `touch config.txt` |
| `cat <file>` | 读取并打印文件内容 | `cat readme.md` |
| `echo <text>` | 打印文本 | `echo hello world` |
| `echo <text> > <file>` | 将文本写入文件（覆盖） | `echo data > log.txt` |
| `rm <file>` | 删除文件 | `rm old.txt` |
| `rm -rf <dir>` | 递归清空目录内所有文件（CH378 不支持删除目录本身） | `rm -rf temp` |

#### 注意事项

- **CH378 目录删除限制**：CH378 固件的 `CMD0H_FILE_ERASE` 命令官方注释明确为"对目录则等待"，即不支持删除目录。`rm -rf` 会递归清空目录树内所有文件，但空目录会保留。
- **长文件名**：CH378 默认返回 FAT 8.3 短文件名（如 `CON~1.JSO`），如需长文件名支持需额外实现 LFN 解析或 CH378 长文件名扩展命令。
- **终端换行**：MCU 中断仅在收到 `` 时回显 `
`，忽略单独的 `
`，以避免终端发送 `
` 时产生重复换行。
