...
Submodel模块三个（Submodel-7（副屏）体积较大，物理上占用两个 Submodel 位置，
因此整机最多只能插入 2 个 Submodel（如 Submodel-7 + 一个普通 Submodel），占用 2 路 UART）
...
- CS43131是一个音频DAC芯片，接了外部22.5782MHz晶振，作为I2S1 Master和CH32H417通信，
  ...
  因此HPOAB是需要一直输出的）。
- CH585F是一个独立主控wireless-model，但是主要是作为一个蓝牙芯片来使用，
  通过SPI4和CH32H417连接，主要职责是连接上位机软件，实现操控的功能，其UART1作为DEBUG。
- CH32H417作为核心为core-model，连接了三个板上的固定按钮：
  - `+`：短按为音量增加，在列表/菜单界面为向上滚动，在调节界面为增加数值；
  - `-`：短按为音量减小，在列表/菜单界面为向下滚动，在调节界面为减小数值；
  - `Enter`：短按为确认/进入，长按为系统唤醒（无开关机功能）。
  核心板还连接了一个以太网口HR911105A，连接了上述四块芯片，负责调度四块芯片和其他模块，
  其UART2作为DEBUG，引出了UART3-8作为连接其他模块的接口，每个UART连接一种模块，
  分配为UART3-Keyboard，UART4-Display，UART5-Power，UART6-Submodel1，
  UART7-Submodel2，UART8-Submodel3。
...
- 主键盘Keyboard-1，大致为40配列，主控通过UART2和CH9329进行连接，
  CH9329是一款串口转HID芯片，转换为HID之后输出到USB-A口，是Device口，作为标准USB HID设备。
...
- 供电模块Power连接了一个6000mAh的电池，并且可以读取电量，可以输出慢充作为充电宝使用，
  支持PD快充也支持无线充电，可以读取当前的供电/充电功率。通过接口给核心模块和其他所有模块供5V电。
...
**Power**、**Submodels**、**CH585F** 之间的所有数据交互。

- **物理层**：UART（115200/8-N-1，Display 模块使用 921600），
...
>
> **Display 访问说明**：UART4 由 V5F 独占访问。V3F 如有显示相关指令，
> 通过 `hardware.c` 中的共享内存 / HSEM 机制通知 V5F，由 V5F 统一打包发送。
...
| DATA[1]      | 模块子类型编号 | 标识该大类下的具体实现/变种，由各模块协议文档定义。 |
...
| ------------- | ----------------- | ------------------------------------------ |
| `0x00`        | 保留              | 系统保留。                                 |
| `0x01`        | Display Module    | 屏幕显示模块。子类型：LCD、墨水屏等。      |
| `0x02`        | Wireless Module   | 蓝牙/无线模块（CH585F）。                  |
| `0x03`        | Power Module      | 供电管理模块。                             |
| `0x04`        | Keyboard Module   | 键盘输入模块。子类型：主键盘、游戏键盘、音乐键盘等。 |
| `0x05`        | Submodel          | 通用配件模块。子类型：指纹、健康监测、NFC、触摸环、RGB、红外、副屏等。 |
| `0x06~0xFF`   | 预留              | 未来扩展。                                 |
