...
```
App Layer (ui_main.c, ui_home.c, ui_software.c, ui_models.c, ui_settings.c, app_*.c, game_*.c)
    |
...
│   ├── ui_home.c/h           -- 首页
│   ├── ui_software.c/h       -- 应用网格（系统应用+本地游戏入口）
│   ├── ui_models.c/h         -- 模块状态
│   ├── ui_settings.c/h       -- 设置列表
│   └── ui_games.c/h          -- 游戏框架（游戏启动/退出/全屏切换）
...
- **侧边栏**：宽200px，高480px，背景色 `#F5F5F5`
  ...
  - 4个菜单项：Main/Software/Models/Options
...
### 4.3 应用界面（ui_software.c）

- 可滚动网格：图标按钮（80x80，圆角16px）+ 文字标签
- 系统应用和本地游戏分类
- 滚动通过拖动实现（记录drag delta，偏移所有widget的y坐标）
- 点击应用图标 → `ui_page_push(&app_xxx_page)` 进入应用页面
- 点击游戏图标 → `ui_page_push(&game_xxx_page)` 进入游戏页面

### 4.4 模块界面（ui_models.c）

- Tab视图：Display/Keyboard/Power/Core/BT-WiFi
- 每个Tab页：卡片式布局，显示模块名称、状态圆点、关键参数

### 4.5 设置界面（ui_settings.c）
...
- 每个应用一个独立页面，统一风格：
  ...
  - 中间内容区（应用特定UI）
  - 底部状态栏（如有需要）
- 返回按钮调用 `ui_page_pop()` 回到应用列表
- 通过UART与核心模块通信

### 4.7 游戏页面（game_*.c）

- 游戏全屏模式：隐藏侧边栏，内容区扩展为800x480
- 页面flags标记 `PAGE_FLAG_FULLSCREEN`
- 游戏逻辑在game_*.c中，每帧通过`ui_page->on_draw()`回调渲染
- 俄罗斯方块：10x20网格，每个格子30x30px
- 2048：4x4网格，每个格子120x120px，圆角12px
- 游戏内返回：点击右上角关闭按钮或按键盘ESC → 确认对话框 → `ui_page_pop()`
