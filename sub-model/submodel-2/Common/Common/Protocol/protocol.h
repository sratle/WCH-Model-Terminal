...
#define MODULE_SUBTYPE_HEALTH   0x02
...
/* Health subcommand IDs (DATA[0] in submodel frames) */

/* CMD_SUB_SET_CONFIG (0x44) sub-commands */
#define HEALTH_SUB_SET_INTERVAL    0x01
#define HEALTH_SUB_START_MONITOR   0x02
#define HEALTH_SUB_STOP_MONITOR    0x03

/* CMD_SUB_GET_STATUS (0x42) sub-commands */
#define HEALTH_SUB_QUERY_DATA      0x00

/* CMD_SUB_DATA_REPORT (0x43) sub-commands */
#define HEALTH_SUB_DATA_REPORT     0x01
...
typedef struct {
    ...
    protocol_frame_t read_frame;
    uint16_t data_idx;
    uint8_t  frame_ready;
    uint8_t  frame_consumed;
} protocol_rx_ctx_t;
