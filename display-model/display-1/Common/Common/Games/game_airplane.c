...
static void ap_draw_hud(void)
{
    ...
    ui_draw_text(AP_AREA_X + 12, AP_AREA_Y + 7,
                 s_ap.buf_time, UI_FONT_BODY, UI_COLOR_WHITE);

    int16_t hp_w = ui_text_width(s_ap.buf_hp, UI_FONT_BODY);
    ui_draw_text(AP_AREA_X + AP_AREA_W/2 - hp_w/2, AP_AREA_Y + 7,
                 s_ap.buf_hp, UI_FONT_BODY,
    ...
    ap_strcat(buf, phase);
    int16_t score_w = ui_text_width(buf, UI_FONT_BODY);
...
static void ap_draw_idle_screen(void)
{
    ...
    ui_draw_text_in_rect(&title_rect, "Airplane", UI_FONT_TITLE,
                         UI_COLOR_PRIMARY, 1);
    ...
    ui_draw_text_in_rect(&inst_rect, "Shoot down enemy planes! Dodge their bullets!",
                         UI_FONT_BODY, UI_COLOR_TEXT_SECONDARY, 1);
    ...
    ui_draw_text_in_rect(&hint_rect, "Touch and drag to move, tap to start",
                         UI_FONT_BODY, UI_COLOR_ACCENT, 1);

    /* Show best score */
    if (s_ap.best > 0) {
        ui_rect_t best_rect = {AP_AREA_X, AP_AREA_Y + AP_AREA_H / 2 + 48, AP_AREA_W, 24};
        ui_draw_text_in_rect(&best_rect, s_ap.buf_best, UI_FONT_BODY,
                             UI_COLOR_ACCENT, 1);
    }
}
...
static void ap_draw_gameover(void)
{
    ...
    ui_draw_text_in_rect(&go_rect, "GAME OVER", UI_FONT_TITLE,
                         UI_COLOR_ACCENT, 1);

    ui_rect_t score_rect = {AP_AREA_X, AP_AREA_Y + AP_AREA_H / 2 - 22, AP_AREA_W, 24};
    ui_draw_text_in_rect(&score_rect, s_ap.buf_gameover, UI_FONT_BODY,
                         UI_COLOR_WHITE, 1);
    ...
    ui_draw_text_in_rect(&info_rect, buf_info, UI_FONT_BODY,
                         UI_COLOR_SECONDARY, 1);

    ui_rect_t hint_rect = {AP_AREA_X, AP_AREA_Y + AP_AREA_H / 2 + 38, AP_AREA_W, 24};
    ui_draw_text_in_rect(&hint_rect, "Tap to restart", UI_FONT_BODY,
                         UI_COLOR_SECONDARY, 1);
...
static void ap_draw_entities_in_clip(const ui_rect_t *clip)
{
    ...
    if (s_ap.dmg_show_timer > 0) {
        ...
        if (ap_rects_overlap(&dr, clip))
            ui_draw_text(dx - 10, dy, "-10", UI_FONT_BODY, UI_HEX(0xE74C3C));
    }
}
