...
static void g4_on_cli_complete(const char *buf, uint16_t len, const char *tag)
{
    ...
    /* Only parse numeric responses (from "get"), skip "OK" (from "set") */
...
static void g4_draw_panel(void)
{
    ...
    /* Score */
    ui_draw_text(G4_PANEL_X, y, "SCORE", UI_FONT_BODY, UI_HEX(0x776E65));
    y += 18;
    ...
    /* Moves */
    ui_draw_text(G4_PANEL_X, y, "MOVES", UI_FONT_BODY, UI_HEX(0x776E65));
    y += 18;
    ...
    /* Best */
    ui_draw_text(G4_PANEL_X, y, "BEST", UI_FONT_BODY, UI_HEX(0x776E65));
    y += 18;
    ui_rect_t best_bg = {G4_PANEL_X, y, G4_PANEL_W, 36};
    ui_draw_fill_round_rect(&best_bg, 6, UI_HEX(0xBBADA0));
    ui_draw_text_in_rect(&best_bg, s_g4.buf_best, UI_FONT_TITLE, UI_COLOR_WHITE, 1);
    y += 50;
...
static void g4_draw_idle_screen(void)
{
    ...
    ui_rect_t title = {G4_GRID_X, G4_GRID_Y + G4_GRID_TOTAL / 2 - 40,
                       G4_GRID_TOTAL, 40};
    ui_draw_text_in_rect(&title, "2048", UI_FONT_TITLE, UI_HEX(0x776E65), 1);
...
static void g4_draw_overlay(const char *title, ui_color_t title_color)
{
    ...
    ui_rect_t sr = {G4_GRID_X, G4_GRID_Y + G4_GRID_TOTAL / 2 + 5,
                    G4_GRID_TOTAL, 24};
    ui_draw_text_in_rect(&sr, buf, UI_FONT_BODY, UI_HEX(0x776E65), 1);
