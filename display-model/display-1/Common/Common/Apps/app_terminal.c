        ...
        if (line_idx < (int16_t)s_term.line_count) {
            ...
            if (line_len > 0) {
                ...
                if (len >= 2 && text[0] == '>' && text[1] == ' ') {
                    ...
                    ui_draw_text(TERM_MARGIN_X, y, "> ", UI_FONT_BODY, TERM_PROMPT_COLOR);
                    if (len > 2) {
                        ...
                        cmd[cmd_len] = '\0';
                        ui_draw_text(TERM_MARGIN_X + ui_text_width("> ", UI_FONT_BODY), y,
                                     cmd, UI_FONT_BODY, TERM_INPUT_COLOR);
                    }
                } else {
                    ...
                    ui_draw_text(TERM_MARGIN_X, y, text, UI_FONT_BODY, color);
                }
            }
        } else if (line_idx == (int16_t)s_term.line_count) {
            /* Input line: draw prompt + input text + cursor */
            ui_draw_text(TERM_MARGIN_X, y, "> ", UI_FONT_BODY, TERM_PROMPT_COLOR);
            int16_t prompt_w = ui_text_width("> ", UI_FONT_BODY);
            ...
            if (s_term.input_len > 0) {
                ...
                if (s_term.cursor_pos > 0) {
                    ...
                    before[blen] = '\0';
                    ui_draw_text(text_x, y, before, UI_FONT_BODY, TERM_INPUT_COLOR);
                    text_x += ui_text_width(before, UI_FONT_BODY);
                }
                ...
                if (s_term.cursor_pos < s_term.input_len) {
                    ...
                    after[alen] = '\0';
                    int16_t cursor_w = 2;
                    ui_draw_text(text_x + cursor_w, y, after, UI_FONT_BODY, TERM_INPUT_COLOR);
                }
            } else {
...
static void term_draw_status(void)
{
    ...
    buf[pos] = '\0';
    ui_draw_text(4, y + 3, buf, UI_FONT_BODY, TERM_STATUS_TEXT);
}
