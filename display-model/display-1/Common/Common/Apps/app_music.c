...
static void music_draw_disc(void)
{
    ...
    ui_draw_text(cx - 6, cy - 8, "*", UI_FONT_TITLE, MUSIC_ACCENT);
}

static void music_draw_info(void)
{
    ui_rect_t info_rect = {20, INFO_Y, LEFT_W - 40, 28};
    ui_draw_text_in_rect(&info_rect, s_track_name, UI_FONT_TITLE, MUSIC_TEXT, 0x11);
    ui_rect_t status_rect = {20, INFO_Y + 30, LEFT_W - 40, 20};
    ui_draw_text_in_rect(&status_rect, s_status_text, UI_FONT_BODY, MUSIC_TEXT_DIM, 0x11);
}
...
static void music_draw_progress(void)
{
    ...
    ui_draw_text(PROG_X, TIME_Y, s_time_pos, UI_FONT_BODY, MUSIC_TEXT_DIM);
    int16_t dw = ui_text_width(s_time_dur, UI_FONT_BODY);
    ui_draw_text(PROG_X + PROG_W - dw, TIME_Y, s_time_dur, UI_FONT_BODY, MUSIC_TEXT_DIM);
}
...
static void music_draw_volume(void)
{
    ...
    ui_draw_text(VOL_X - 36, VOL_Y + 4, "Vol", UI_FONT_BODY, MUSIC_TEXT_DIM);
    ui_draw_text(VOL_X + VOL_W + 6, VOL_Y + 4, s_vol_text, UI_FONT_BODY, MUSIC_TEXT_DIM);
}

static void music_draw_speaker_row(void)
{
    ui_draw_text(SPK_BTN_X - 36, SPK_Y + 10, "Out", UI_FONT_BODY, MUSIC_TEXT_DIM);
}
...
static void music_draw_playlist(void)
{
    ...
    ui_draw_text(PL_X + 10, PL_Y + 6, hdr, UI_FONT_BODY, MUSIC_TEXT_DIM);
    ...
    for (int16_t i = 0; i <= PL_VISIBLE && (first + i) < s_playlist.count; i++) {
        ...
        if (idx == s_playlist.current) {
            ...
            /* Playing indicator */
            ui_draw_text(PL_X + 10, iy + 8, ">", UI_FONT_BODY, MUSIC_PL_CUR);
            ui_draw_text(PL_X + 24, iy + 8, s_playlist.names[idx],
                         UI_FONT_BODY, MUSIC_PL_CUR);
        } else {
            ui_draw_text(PL_X + 24, iy + 8, s_playlist.names[idx],
                         UI_FONT_BODY, MUSIC_TEXT_DIM);
        }
