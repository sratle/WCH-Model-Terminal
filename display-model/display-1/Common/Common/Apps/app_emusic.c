...
static void draw_white_key(uint8_t wk_idx, bool pressed)
{
    ...
    int16_t tw = ui_text_width(name, UI_FONT_BODY);
    int16_t tx = r.x + (r.w - tw) / 2;
    ui_draw_text(tx, r.y + r.h - 18, name, UI_FONT_BODY,
...
static void draw_button(uint8_t btn_idx, bool pressed)
{
    ...
    int16_t tw = ui_text_width(label, UI_FONT_BODY);
    int16_t tx = r.x + (r.w - tw) / 2;
    int16_t ty = r.y + (r.h - 12) / 2;
    ui_draw_text(tx, ty, label, UI_FONT_BODY,
...
static void draw_fader(uint8_t f_idx, uint16_t value)
{
    ...
    /* Label above */
    int16_t label_x = track.x + (track.w - ui_text_width(s_fader_labels[f_idx], UI_FONT_BODY)) / 2;
    ui_draw_text(label_x, track.y - 18, s_fader_labels[f_idx],
                 UI_FONT_BODY, EMU_TEXT);

    /* Percentage below */
    char pct_text[8];
    snprintf(pct_text, sizeof(pct_text), "%d%%", pct);
    int16_t pct_x = track.x + (track.w - ui_text_width(pct_text, UI_FONT_BODY)) / 2;
    ui_draw_text(pct_x, track.y + track.h + 6, pct_text,
                 UI_FONT_BODY, EMU_TEXT_DIM);
}
...
void app_emusic_init(void)
{
    ...
    /* Initialize toggle button */
    {
        ...
        ui_button_init(&s_toggle_btn, &btn_rect, "Start", UI_FONT_TITLE);
        ui_button_set_colors(&s_toggle_btn, EMU_START_BG, EMU_START_BG_PRESSED, UI_COLOR_WHITE);
