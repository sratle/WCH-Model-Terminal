...
static void sd_page_draw(ui_page_t *page, ui_rect_t *dirty)
{
    ...
    if (dirty_top < UI_SCREEN_HEIGHT - SD_BAR_H && dirty->x < SD_LEFT_W) {
        /* Section labels */
        ui_draw_text(12, SD_MODE_Y - 2, "Display Mode", UI_FONT_BODY, SD_LABEL);
        ui_draw_text(12, SD_LIST_LABEL_Y, "BMP Files", UI_FONT_BODY, SD_LABEL);
        ...
        if (dirty_bot > SD_LIST_Y) {
            ...
            for (int16_t vis = 0; vis < SD_VISIBLE_ITEMS; vis++) {
                ...
                ui_draw_text(SD_LIST_X + 8, item_y + 8, icon, UI_FONT_BODY, icon_color);

                /* File name */
                ui_draw_text(SD_LIST_X + 36, item_y + 8, s_bmp_files[idx].name,
                             UI_FONT_BODY, SD_ITEM_TEXT);
                ...
                if (!s_bmp_files[idx].is_dir && s_bmp_files[idx].size > 0) {
                    ...
                    int16_t sw = ui_text_width(size_text, UI_FONT_BODY);
                    ui_draw_text(SD_LIST_X + SD_LIST_W - sw - 10, item_y + 8,
                                 size_text, UI_FONT_BODY, SD_ITEM_SIZE);
                }
            }
    ...
    if (dirty_bot > SD_STATUS_Y && dirty->x + dirty->w > SD_RIGHT_X) {
        ...
        if (s_status_len > 0) {
            ...
            ui_draw_text(SD_RIGHT_X + 10, SD_STATUS_Y + 8,
                         "Loading...", UI_FONT_BODY, SD_STATUS_TEXT);
        }
    }
    ...
    if (dirty_bot > SD_BAR_Y) {
        ...
        ui_draw_text(12, SD_BAR_Y + 8, s_bar_msg, UI_FONT_BODY, SD_BAR_TEXT);
    }
}
...
void app_subdisplay_init(void)
{
    ...
    /* Mode buttons */
    {
        ...
        ui_button_init(&btn_mode_status, &r1, "Status Mode", UI_FONT_BODY);
        ...
        ui_button_init(&btn_mode_image, &r2, "Image Mode", UI_FONT_BODY);
    ...
    /* Refresh button */
    {
        ...
        ui_button_init(&btn_refresh, &r, "Refresh Status", UI_FONT_BODY);
    ...
    /* Send button */
    {
        ...
        ui_button_init(&btn_send, &r, "Send to SubDisplay", UI_FONT_BODY);
