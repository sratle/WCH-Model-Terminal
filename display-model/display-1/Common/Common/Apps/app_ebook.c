...
* Author             : LCD Model Team (paged rewrite)
* Version            : V2.0.0
* Date               : 2026/07/19
* Description        : EBook reader app — PAGED BYTE-STREAM.
*                      V2.0: no whole-file loading (the old 20KB static
*                      buffer is gone).  The Core CLI `read <file> <off> <len>`
*                      returns an exact byte range; the app renders a page
*                      from a 3KB chunk and records the byte offset where the
*                      next page starts (page offset stack), so RAM usage is
*                      flat for any book size and long books finally work.
********************************************************************************/
#include "app_ebook.h"
...
#define EB_CHUNK            2048    /* bytes per Core `read` request:
                                     * ~1 rendered page (~1.7KB worst case)
                                     * + margin covering the boundary line.
                                     * Per-page fetch: one page per request,
...
static const eb_font_cfg_t s_font_cfgs[] = {
    { UI_FONT_BODY, 22, 10, 8 },   /* Small  */
    { UI_FONT_TITLE, 30, 12, 10 },  /* Medium */
    { UI_FONT_TITLE, 34, 14, 12 },  /* Large  */
};
 ...
 * EB_CHUNK bytes (page + boundary margin) at the page start offset. */
static void eb_ensure_chunk(void)
{
...
/*=============================================================================
 *  Text rendering helpers (with real glyph widths)
 *=============================================================================*/
                                  ...
                                  int16_t line_h, int16_t max_y)
{
    ...
    while (p < end) {
        const ui_glyph_t *g = ui_font_glyph(font, (uint8_t)*p);
        int16_t char_w = g ? g->advance : font->height / 2;
                               ...
                               uint32_t buf_len, uint32_t start)
{
    ...
    while (p < end && lines_drawn < lines_per_page && draw_y + line_h <= max_y) {
        ...
        if (is_md && line_len > 0) {
            if (line_start[0] == '#') {
                int heading_level = 0;
                while (heading_level < (int)line_len && line_start[heading_level] == '#')
                    heading_level++;
                if (heading_level <= 3) {
                    const char *h_text = line_start + heading_level;
                    while (h_text < eol && *h_text == ' ') h_text++;
                    uint16_t h_len = (uint16_t)(eol - h_text);

                    ui_draw_hline(text_x, draw_y + line_h, text_w, th->hdr_line);
            ...
            if ((line_start[0] == '-' || line_start[0] == '*') &&
                ...
                const char *li_text = line_start + 2;
            ...
            if (quote1) {
                ...
                if (colon) {
                    ...
                    if (pre_len > 0) {
                        ...
                        tmp[cl] = '\0';
                        ui_draw_text(cx, draw_y, tmp, fc->font, text_color);
                    ...
                    }
                    {
                        ...
                        tmp[cl] = '\0';
                        ui_draw_text(cx, draw_y, tmp, fc->font, th->key_text);
                        cx += ui_text_width(tmp, fc->font);
                    }
    ...
    if (dl < EB_LEFT_W && db > EB_LIST_Y) {
        ...
        for (int16_t vis = 0; vis < EB_VISIBLE; vis++) {
            ...
            ui_draw_fill_round_rect(
                &(ui_rect_t){EB_LIST_X + 6, iy + 6, 48, 24}, 3, th->badge_bg);
            ui_draw_text(EB_LIST_X + 10, iy + 9,
                         s_files[idx].ext, UI_FONT_BODY, th->badge_text);
...
void app_ebook_init(void)
{
...

    {
        ...
        ui_button_init(&btn_refresh, &r, "Reload", UI_FONT_BODY);
...

    {
        ...
        ui_button_init(&btn_prev, &r, "Prev", UI_FONT_BODY);
...

    {
        ...
        ui_button_init(&btn_next, &r, "Next", UI_FONT_BODY);
...

    {
        ...
        ui_button_init(&btn_theme, &r, "Theme", UI_FONT_BODY);
...

    {
        ...
        ui_button_init(&btn_font, &r, "Font", UI_FONT_BODY);
...

    {
        ...
        ui_button_init(&btn_back_list, &r, "Back", UI_FONT_BODY);
