...
static void bt_draw_chart(void)
{
    ...
    if (n == 0) {
        ui_draw_text(BT_ROW_X + BT_ROW_W / 2 - 50, BT_CHART_Y + BT_CHART_H / 2 - 8,
                     "No traffic yet", UI_FONT_BODY, UI_COLOR_TEXT_SECONDARY);
        return;
    }
    ...
    ui_draw_vline(px, py, ph, UI_COLOR_TEXT_SECONDARY);
    {
        ...
        ui_draw_text(BT_ROW_X + 8, py - 6, maxs, UI_FONT_BODY, UI_COLOR_TEXT_SECONDARY);
    ...
    for (i = 0; i < n; i++) {
        ...
        /* Value label above the bar */
        {
            ...
            int16_t tw = ui_text_width(vs, UI_FONT_BODY);
            int16_t tx = bx + (bar_w - tw) / 2;
            if (tx < px) tx = px;
            int16_t ty = by - 14;
            if (ty < py) ty = py;
            ui_draw_text(tx, ty, vs, UI_FONT_BODY, UI_COLOR_TEXT_PRIMARY);
        }
    }
}
