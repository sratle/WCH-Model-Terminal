    ...
    if (dl < EB_LEFT_W && db > EB_LIST_Y) {
        ...
        for (int16_t vis = 0; vis < EB_VISIBLE; vis++) {
            ...
            ui_draw_text(EB_LIST_X + 10, iy + 9,
                         s_files[idx].ext, UI_FONT_BODY, fg);
...
static void view_touch_event(ui_widget_t *w, ui_event_t *e)
{
    ...
    if (e->type == UI_EVENT_SWIPE_LEFT || e->type == UI_EVENT_SWIPE_UP) {
        ...
        /* Left click / touch tap: right half = next, left half = prev. */
        else if (e->touch.x > EB_VIEW_X + EB_VIEW_W / 2) {
            eb_next_page();
        } else {
            eb_prev_page();
        }
...
static bool eb_page_event(ui_page_t *page, ui_event_t *e)
{
    ...
    if (s_viewing) {
        ...
        /* WASD-style paging: 'A' = previous, 'D' = next (pairs with arrows). */
        if (e->type == UI_EVENT_KEY_DOWN && (e->char_code == 'a' || e->char_code == 'A')) {
            eb_prev_page();
            return true;
        }
        if (e->type == UI_EVENT_KEY_DOWN && (e->char_code == 'd' || e->char_code == 'D')) {
            eb_next_page();
            return true;
        }
    } else {
...
void app_ebook_init(void)
{
...

    {
        ...
        ui_button_init(&btn_back_list, &r, "Books", UI_FONT_BODY);
