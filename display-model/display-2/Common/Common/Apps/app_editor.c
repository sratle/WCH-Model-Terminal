...
static void editor_draw_gutter(void)
{
    ...
    for (int i = 0; i < EDIT_VISIBLE_LINES; i++) {
        ...
        ui_draw_text(EDIT_GUTTER_W - nw - 8, y, nb, UI_FONT_BODY, UI_COLOR_BLACK);
    }
    ...
    if (s_ed.has_content &&
        ...
        ui_draw_text(4, my, ">", UI_FONT_BODY, UI_COLOR_BLACK);
    }
}
    ...
    for (int i = 0; i < EDIT_VISIBLE_LINES; i++) {
        ...
        if (s_ed.has_selection) {
            ...
            if (start < sel_e && end > sel_s) {
                ...
                sel_txt[st_len] = '\0';
                ui_draw_text(sel_x_start, y, sel_txt, UI_FONT_BODY, UI_COLOR_WHITE);
                ...
                if (st_off > 0) {
                    ...
                    pre[st_off] = '\0';
                    ui_draw_text(EDIT_TEXT_X + 8, y, pre, UI_FONT_BODY, UI_COLOR_BLACK);
                }
                {
                    uint16_t suf_off = line_sel_end - start;
                    if (suf_off < ll) {
                        ui_draw_text(sel_x_end, y, lb + suf_off, UI_FONT_BODY, UI_COLOR_BLACK);
                    }
                }
                continue;
            }
        ...
        ui_draw_text(EDIT_TEXT_X + 8, y, lb, UI_FONT_BODY, UI_COLOR_BLACK);
    }
...
static void editor_draw_status(void)
{
    ...
    if (s_ed.is_modified) {
        int16_t nw = ui_text_width(s_ed.file_name, UI_FONT_BODY);
    ...
    ui_draw_text(UI_SCREEN_WIDTH - sw - 12, EDIT_STATUS_Y + 6,
                 s_status_buf, UI_FONT_BODY, UI_COLOR_BLACK);
}
