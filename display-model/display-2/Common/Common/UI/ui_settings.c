...
static const cfg_item_def_t s_display_items[] = {
    { "0102", "operationsound", "Operation Sound",   0, 1,   1 },
    { "0102", "gamebgm",        "Game BGM",          0, 1,   1 },
};
...
static const char * const s_cfg_modules[CFG_MODULE_COUNT] = {
    "0000", "0102", "0201", "0301", "0502", "0505", "0506", "0507"
};
...
static void slider_change(ui_widget_t *w, int16_t value)
{
    ...
    if (strcmp(item->module_key, "0102") == 0 && strcmp(item->key, "operationsound") == 0) {
        g_settings.operation_sound = (value != 0);
    }
    if (strcmp(item->module_key, "0102") == 0 && strcmp(item->key, "gamebgm") == 0) {
...
void ui_settings_init(void)
{
    ...
    ui_label_init(&lbl_title, &title_rect, "Settings", UI_FONT_TITLE);
    ...
    ui_button_init(&btn_save, &save_rect, "Save", UI_FONT_BODY);
    ...
    for (uint8_t i = 0; i < SETTINGS_MAX_ITEMS; i++) {
        ui_rect_t r = {0, 0, 0, 0};
        ui_list_item_init(&s_items[i], &r, "", UI_FONT_BODY);
        s_items[i].base.flags &= ~UI_WIDGET_FLAG_VISIBLE;
    ...
    ui_list_item_init(&item_about, &about_rect, "About", UI_FONT_BODY);
    item_about.show_divider = false;
    item_about.base.flags &= ~UI_WIDGET_FLAG_VISIBLE;
    ui_rect_t btn_about_rect = {0, 0, 80, 34};
    ui_button_init(&btn_about_info, &btn_about_rect, "Info", UI_FONT_BODY);
