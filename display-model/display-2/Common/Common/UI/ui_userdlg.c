...
* Description        : User dialog page (1bpp e-ink).
...
static void ud_page_draw(ui_page_t *page, ui_rect_t *dirty)
{
    ...
    /* Title bar + back button + title (复用模板) */
    ui_app_page_draw(page, dirty);
    ...
    ui_draw_fill_round_rect(&panel, 8, UI_COLOR_WHITE);
    ui_draw_round_rect_border(&panel, 8, UI_COLOR_BLACK, 2);

    if (!s_ud_st.info_mode) {
        /* ---- 登录模式（左列 + 右侧 PIN Pad 组件） ---- */
        ui_draw_text(dx + 20, dy + 16, "User Login", UI_FONT_TITLE, UI_COLOR_BLACK);
        ...
        /* Login button */
        ui_rect_t ok_r = {dx + 20, dy + UD_PANEL_H - 52, 96, 36};
        ui_draw_fill_round_rect(&ok_r, 6, UI_COLOR_BLACK);
        ui_draw_text(ok_r.x + 24, ok_r.y + 10, "Login", UI_FONT_BODY, UI_COLOR_WHITE);
    } else {
        /* ---- 信息模式（已登录） ---- */
        char line[64];

        ui_draw_text(dx + 20, dy + 16, "Current User", UI_FONT_TITLE, UI_COLOR_BLACK);

        snprintf(line, sizeof(line), "Name: %s", g_disp_state.user_name);
        ui_draw_text(dx + 20, dy + 52, line, UI_FONT_TITLE, UI_COLOR_BLACK);
        ...
        snprintf(line, sizeof(line), "FP:  %s",
                 (u && u->fp[0]) ? u->fp : "(none)");
        ui_draw_text(dx + 20, dy + 96, line, UI_FONT_BODY, UI_COLOR_BLACK);

        snprintf(line, sizeof(line), "NFC: %s",
                 (u && u->nfc[0]) ? u->nfc : "(none)");
        ui_draw_text(dx + 20, dy + 124, line, UI_FONT_BODY, UI_COLOR_BLACK);
        ...
        /* Logout button */
        ui_rect_t lo_r = {dx + UD_PANEL_W - 116, dy + UD_PANEL_H - 52, 96, 36};
        ui_draw_fill_round_rect(&lo_r, 6, UI_COLOR_BLACK);
        ui_draw_text(lo_r.x + 18, lo_r.y + 10, "Logout", UI_FONT_BODY, UI_COLOR_WHITE);
    }
}
...
static void ud_touch_event(ui_widget_t *w, ui_event_t *e)
{
    ...
    if (!s_ud_st.info_mode) {
        ...
        ui_rect_t lo_r = {dx + UD_PANEL_W - 116, dy + UD_PANEL_H - 52, 96, 36};
        if (ui_widget_hit_test((ui_widget_t *)&lo_r, e->touch.x, e->touch.y)) {
            ud_logout();
            return;
        }
    }
...
static bool ud_page_event(ui_page_t *page, ui_event_t *e)
{
    ...
    if (!s_ud_st.info_mode) {
        ...
        if (e->type == UI_EVENT_KEY_DOWN || e->type == UI_EVENT_KEY_LONG_REPEAT) {
            if (e->key.code == UI_KEY_LEFT && s_ud_st.user_count > 0) {
                s_ud_st.user_sel--;
                if (s_ud_st.user_sel < 0) s_ud_st.user_sel = s_ud_st.user_count - 1;
                ud_invalidate();
                return true;
            }
            if (e->key.code == UI_KEY_RIGHT && s_ud_st.user_count > 0) {
                s_ud_st.user_sel++;
                if (s_ud_st.user_sel >= s_ud_st.user_count) s_ud_st.user_sel = 0;
                ud_invalidate();
                return true;
