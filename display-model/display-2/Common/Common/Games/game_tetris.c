...
{
    if (val == 0) return UI_COLOR_WHITE;    /* Empty cell = white background */
    return tet_piece_color(val);
}

static ui_color_t tet_cell_border_color(uint8_t val)
{
    if (val == 0) return UI_COLOR_WHITE;    /* Empty cell border = white */
    return UI_COLOR_WHITE;                  /* Filled cell border = white (outline) */
}

static ui_color_t tet_cell_highlight_color(uint8_t val)
{
    if (val == 0) return UI_COLOR_WHITE;    /* Empty cell highlight = white */
    return UI_COLOR_WHITE;                  /* Filled cell highlight = white */
}
...
static void tet_draw_piece(void)
{
    ...
    if (ghost != s_tet.cur_row) {
        ui_color_t ghost_fill = UI_COLOR_WHITE;    /* Ghost = white fill */
        ui_color_t ghost_border = UI_COLOR_BLACK;  /* Ghost = black border only */
        for (int i = 0; i < 4; i++) {
            int8_t cr, cc;
            tet_piece_cell(s_tet.cur_piece, s_tet.cur_rot, i, &cr, &cc);
            int br = ghost + cr;
...
static void tet_draw_preview(void)
{
    ...
    ui_draw_fill_round_rect(&bg, TET_INFO_BOX_R, UI_COLOR_WHITE);    /* White bg for preview */
...
{
    /* Score */
    ui_draw_text(TET_PANEL_X, TET_SCORE_LABEL_Y, "SCORE", &font_montserrat_12, UI_COLOR_BLACK);
    ui_rect_t score_bg = {TET_PANEL_X, TET_SCORE_BOX_Y, TET_PANEL_W, TET_INFO_BOX_H};
    ui_draw_fill_round_rect(&score_bg, TET_INFO_BOX_R, UI_COLOR_BLACK);
    ui_draw_text_in_rect(&score_bg, s_tet.buf_score, &font_montserrat_16, UI_COLOR_WHITE, 1);

    /* Level */
    ui_draw_text(TET_PANEL_X, TET_LEVEL_LABEL_Y, "LEVEL", &font_montserrat_12, UI_COLOR_BLACK);
    ui_rect_t level_bg = {TET_PANEL_X, TET_LEVEL_BOX_Y, TET_PANEL_W, TET_INFO_BOX_H};
    ui_draw_fill_round_rect(&level_bg, TET_INFO_BOX_R, UI_COLOR_BLACK);
    ui_draw_text_in_rect(&level_bg, s_tet.buf_level, &font_montserrat_16, UI_COLOR_WHITE, 1);

    /* Lines */
    ui_draw_text(TET_PANEL_X, TET_LINES_LABEL_Y, "LINES", &font_montserrat_12, UI_COLOR_BLACK);
    ui_rect_t lines_bg = {TET_PANEL_X, TET_LINES_BOX_Y, TET_PANEL_W, TET_INFO_BOX_H};
    ui_draw_fill_round_rect(&lines_bg, TET_INFO_BOX_R, UI_COLOR_BLACK);
    ui_draw_text_in_rect(&lines_bg, s_tet.buf_lines, &font_montserrat_16, UI_COLOR_WHITE, 1);

    /* Next label */
    ui_draw_text(TET_PANEL_X, TET_NEXT_LABEL_Y, "NEXT", &font_montserrat_12, UI_COLOR_BLACK);
    tet_draw_preview();
...
static void tet_draw_idle_screen(void)
{
    ...
    ui_draw_fill_rect(&area, UI_COLOR_WHITE);
    ...
    ui_draw_text_in_rect(&title, "TETRIS", &font_montserrat_16, UI_COLOR_BLACK, 1);
...
static void tet_draw_gameover_overlay(void)
{
    ...
    ui_rect_t title = {TET_GRID_X, TET_GRID_Y + TET_ROWS * TET_CELL / 2 - 30,
                       TET_COLS * TET_CELL, 30};
    ui_draw_text_in_rect(&title, "GAME OVER", &font_montserrat_16, UI_COLOR_BLACK, 1);
...
static void tet_game_update(ui_page_t *page, uint32_t dt_ms)
{
    (void)page;
    tet_game_tick();
}
