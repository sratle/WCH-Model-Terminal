...
static const ui_color_t s_ball_colors[] = {
    UI_COLOR_BLACK,  /* Soft pink */
    UI_COLOR_BLACK,  /* Mint */
    UI_COLOR_BLACK,  /* Soft yellow */
    UI_COLOR_BLACK,  /* Light mint */
    UI_COLOR_BLACK,  /* Soft purple */
    UI_COLOR_BLACK,  /* Soft blue */
    UI_COLOR_BLACK,  /* Light pink */
    UI_COLOR_BLACK,  /* Light green */
};
...
static void tb_draw_hud(void)
{
    /* HUD background bar */
    ui_rect_t hud = {TB_AREA_X, TB_AREA_Y + TB_AREA_H - 32, TB_AREA_W, 32};
    ui_draw_fill_rect(&hud, UI_COLOR_BLACK);
    ui_draw_hline(TB_AREA_X, TB_AREA_Y + TB_AREA_H - 32, TB_AREA_W, UI_COLOR_BLACK);

    /* Score (left) */
    ui_draw_text(TB_AREA_X + 12, TB_AREA_Y + TB_AREA_H - 26,
                 s_tb.buf_score, &font_montserrat_12, UI_COLOR_WHITE);
    ...
    ui_draw_text(TB_AREA_X + TB_AREA_W - fps_w - 12, TB_AREA_Y + TB_AREA_H - 26,
                 s_tb.buf_fps, &font_montserrat_12, UI_COLOR_WHITE);
}

static void tb_draw_idle_screen(void)
{
    /* Decorative background circles */
    ui_draw_fill_circle(TB_AREA_W / 4, TB_AREA_Y + TB_AREA_H / 3, 30, UI_COLOR_BLACK);
    ui_draw_fill_circle(TB_AREA_W * 3 / 4, TB_AREA_Y + TB_AREA_H * 2 / 3, 40, UI_COLOR_BLACK);
    ui_draw_fill_circle(TB_AREA_W / 2, TB_AREA_Y + TB_AREA_H / 2 + 40, 25, UI_COLOR_BLACK);
...
static void tb_draw_gameover(void)
{
    ...
    ui_draw_fill_rect(&overlay, UI_COLOR_BLACK);
 ...
 *  Page Callbacks
 *
 *  on_update: called once per frame by UI_Tick for game logic.
 *  on_draw:   called per-row by compositing renderer for game content.
 *             Drawing primitives auto-clip to the current target row.
 *=============================================================================*/
...
static void tb_touch_event(ui_widget_t *w, ui_event_t *e)
{
    ...
    if (e->type == UI_EVENT_CLICK) {
        tb_on_press(e->touch.x, e->touch.y);
    }
}
...
/* Per-frame game logic update (called once by UI_Tick) */
static void tb_game_update(ui_page_t *page, uint32_t dt_ms)
{
    (void)page;
    tb_update();
}

/* Per-row game rendering (called by compositing renderer for each dirty row) */
...
void game_touchball_init(void)
{
    ...
    ui_page_set_callbacks(&s_game_touchball.page, tb_game_enter, NULL,
                          tb_game_draw, NULL);
    ui_page_set_update_cb(&s_game_touchball.page, tb_game_update);
