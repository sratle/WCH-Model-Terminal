...
{
    if (val == 0) return UI_COLOR_WHITE;    /* Empty cell = white */
    return UI_COLOR_BLACK;                  /* Filled cell = black */
}

static ui_color_t g4_tile_fg(uint16_t val)
{
    if (val == 0) return UI_COLOR_WHITE;    /* No text on empty */
    return UI_COLOR_WHITE;                  /* White text on black bg */
}
...
static void g4_draw_tile(int16_t x, int16_t y, uint16_t val)
{
    ...
    char buf[8];
    g4_itoa(val, buf);
    const ui_font_t *font = UI_FONT_TITLE;
    if (val >= 16384) font = UI_FONT_BODY;
    else if (val >= 1024) font = UI_FONT_BODY;
...
static void g4_draw_grid(void)
{
    ...
    ui_draw_fill_round_rect(&bg, 10, UI_COLOR_BLACK);
...
static void g4_draw_panel(void)
{
    ...
    /* Score */
    ui_draw_text(G4_PANEL_X, y, "SCORE", UI_FONT_BODY, UI_COLOR_BLACK);
    y += 18;
    ui_rect_t score_bg = {G4_PANEL_X, y, G4_PANEL_W, 36};
    ui_draw_fill_round_rect(&score_bg, 6, UI_COLOR_BLACK);
    ui_draw_text_in_rect(&score_bg, s_g4.buf_score, UI_FONT_TITLE, UI_COLOR_WHITE, 1);
    y += 50;

    /* Moves */
    ui_draw_text(G4_PANEL_X, y, "MOVES", UI_FONT_BODY, UI_COLOR_BLACK);
    y += 18;
    ui_rect_t moves_bg = {G4_PANEL_X, y, G4_PANEL_W, 36};
    ui_draw_fill_round_rect(&moves_bg, 6, UI_COLOR_BLACK);
    ui_draw_text_in_rect(&moves_bg, s_g4.buf_moves, UI_FONT_TITLE, UI_COLOR_WHITE, 1);
    y += 50;

    /* Undo button */
    ui_color_t undo_bg = s_g4.undo_available ? UI_COLOR_BLACK : UI_COLOR_BLACK;
    ...
    ui_draw_text_in_rect(&undo_r, "UNDO", UI_FONT_BODY, undo_fg, 1);
...
static void g4_draw_idle_screen(void)
{
    ...
    ui_draw_fill_round_rect(&hint, 10, UI_COLOR_BLACK);

    ui_rect_t title = {G4_GRID_X, G4_GRID_Y + G4_GRID_TOTAL / 2 - 40,
                       G4_GRID_TOTAL, 40};
    ui_draw_text_in_rect(&title, "2048", UI_FONT_TITLE, UI_COLOR_WHITE, 1);

    ui_rect_t sub = {G4_GRID_X, G4_GRID_Y + G4_GRID_TOTAL / 2,
                     G4_GRID_TOTAL, 30};
    ui_draw_text_in_rect(&sub, "Swipe to start", UI_FONT_BODY,
                         UI_COLOR_WHITE, 1);
}
...
static void g4_draw_overlay(const char *title, ui_color_t title_color)
{
    ...
    ui_draw_fill_round_rect(&overlay, 10, UI_COLOR_BLACK);

    ui_rect_t tr = {G4_GRID_X, G4_GRID_Y + G4_GRID_TOTAL / 2 - 30,
                    G4_GRID_TOTAL, 30};
    ui_draw_text_in_rect(&tr, title, UI_FONT_TITLE, title_color, 1);
    ...
    ui_rect_t hr = {G4_GRID_X, G4_GRID_Y + G4_GRID_TOTAL / 2 + 35,
                    G4_GRID_TOTAL, 24};
    ui_draw_text_in_rect(&hr, "Swipe to restart", UI_FONT_BODY,
                         UI_COLOR_WHITE, 1);
}
...
static void g4_game_draw(ui_page_t *page, ui_rect_t *dirty)
{
    ...
    if (s_g4.state == G4_STATE_IDLE) {
        ...
        if (s_g4.state == G4_STATE_GAMEOVER) {
            g4_draw_overlay("Game Over!", UI_COLOR_WHITE);
        } else if (s_g4.state == G4_STATE_WIN) {
            g4_draw_overlay("You Win!", UI_COLOR_WHITE);
        }
    }
}
