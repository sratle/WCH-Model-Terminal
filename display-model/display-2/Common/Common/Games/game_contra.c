...
#define CT_BG_SKY           UI_COLOR_WHITE    /* Sky background */
#define CT_BG_GROUND        UI_COLOR_BLACK    /* Ground */
#define CT_BG_GROUND_TOP    UI_COLOR_BLACK    /* Grass line */
#define CT_PLATFORM_COLOR   UI_COLOR_BLACK    /* Platform */
#define CT_PLATFORM_TOP     UI_COLOR_BLACK    /* Platform grass */
#define CT_PLAYER_COLOR     UI_COLOR_BLACK    /* Player body */
#define CT_PLAYER_HEAD      UI_COLOR_BLACK    /* Player head */
#define CT_PLAYER_GUN       UI_COLOR_BLACK    /* Gun */
#define CT_ENEMY_COLOR      UI_COLOR_BLACK    /* Enemy body */
#define CT_ENEMY_HEAD       UI_COLOR_BLACK    /* Enemy head */
#define CT_BOSS_COLOR       UI_COLOR_BLACK    /* Boss body */
#define CT_BOSS_HEAD        UI_COLOR_BLACK    /* Boss head */
#define CT_BULLET_COLOR     UI_COLOR_BLACK    /* Player bullet */
#define CT_E_BULLET_COLOR   UI_COLOR_BLACK    /* Enemy bullet */
#define CT_HUD_BG           UI_COLOR_BLACK    /* HUD background */
#define CT_CTRL_BG          UI_COLOR_BLACK    /* Control button bg */
#define CT_CTRL_BG_PRESSED  UI_COLOR_WHITE    /* Control button pressed */
#define CT_CTRL_ALPHA       120                 /* Control overlay alpha (unused on e-ink) */
#define CT_PARTICLE_COLOR   UI_COLOR_BLACK    /* Explosion particle */
...
static void ct_draw_game(void)
{
    ...
    for (int i = 0; i < 30; i++) {
        ...
        ui_draw_pixel(sx, sy, UI_COLOR_BLACK);
    }
    ...
    for (int i = 0; i < 8; i++) {
        ...
        ui_draw_fill_rect(&mr, UI_COLOR_BLACK);
    }
    ...
    for (int i = 0; i < CT_MAX_ENEMIES; i++) {
        ...
        if (e->type == CT_ENEMY_BOSS) {
            int16_t bar_w = ew;
            int16_t bar_h = 4;
            ct_draw_world_rect(e->x, e->y - 8, bar_w, bar_h, UI_COLOR_BLACK);
            int16_t fill_w = (int16_t)((int32_t)bar_w * e->hp / CT_BOSS_HP);
            ct_draw_world_rect(e->x, e->y - 8, fill_w, bar_h, UI_COLOR_BLACK);
        }

        /* Gun */
        int16_t gun_y = e->y + eh / 3;
        if (e->facing_right) {
            ct_draw_world_rect(e->x + ew, gun_y, 6, 3, UI_COLOR_BLACK);
        } else {
            ct_draw_world_rect(e->x - 6, gun_y, 6, 3, UI_COLOR_BLACK);
        }
    }
    ...
    /* Control overlays - B/W on e-ink: pressed = white, unpressed = black */
    /* D-pad */
    ui_color_t dpad_c = UI_COLOR_BLACK;
    ui_color_t dpad_up_c = s_ct.input_up ? UI_COLOR_WHITE : dpad_c;
    ui_color_t dpad_dn_c = s_ct.input_down ? UI_COLOR_WHITE : dpad_c;
    ui_color_t dpad_lt_c = s_ct.input_left ? UI_COLOR_WHITE : dpad_c;
    ui_color_t dpad_rt_c = s_ct.input_right ? UI_COLOR_WHITE : dpad_c;
    ...
    /* Action buttons */
    ui_color_t jump_c = s_ct.input_jump ? UI_COLOR_WHITE : dpad_c;
    ui_color_t shoot_c = s_ct.input_shoot ? UI_COLOR_WHITE : dpad_c;
...
static void ct_draw_idle_overlay(void)
{
    ...
    ui_color_t obg = UI_COLOR_WHITE;    /* Overlay bg on e-ink */
    ui_draw_fill_rect(&overlay, obg);

    ui_rect_t title = {0, CT_AREA_Y + CT_AREA_H / 2 - 40, CT_AREA_W, 30};
    ui_draw_text_in_rect(&title, "CONTRA", &font_montserrat_16, UI_COLOR_BLACK, 1);
...
static void ct_draw_gameover_overlay(void)
{
    ...
    ui_draw_text_in_rect(&sr, buf, &font_montserrat_12, UI_COLOR_BLACK, 1);
...
static void ct_touch_event(ui_widget_t *w, ui_event_t *e)
{
    ...
    if (e->source == UI_INPUT_TOUCH && e->type == UI_EVENT_DOWN) {
        uint8_t btns = ct_hit_test(e->touch.x, e->touch.y);
        if (e->touch.id < UI_MAX_TOUCH_POINTS) {
            s_touch_btns[e->touch.id] = btns;
        }
        ct_recalc_inputs();
        return;
    }
...
static void ct_game_update(ui_page_t *page, uint32_t dt_ms)
{
    (void)page;
    ct_update();
}
