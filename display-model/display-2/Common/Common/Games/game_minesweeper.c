...
#define MINE_GRID_AREA_W       340    /* Left area width for grid (adapted for 648px) */
#define MINE_GRID_X_BASE       10

/* Right panel */
#define MINE_PANEL_X           358
#define MINE_PANEL_W           280
...
#define MINE_CELL_HIDDEN        UI_COLOR_BLACK   /* Unrevealed cell = black (solid) */
#define MINE_CELL_HIDDEN_HI     UI_COLOR_WHITE   /* Highlight on hidden cell */
#define MINE_CELL_HIDDEN_BD     UI_COLOR_WHITE   /* Border of hidden cell */
#define MINE_CELL_REVEALED      UI_COLOR_WHITE   /* Revealed empty cell = white */
#define MINE_CELL_REVEALED_BD   UI_COLOR_BLACK   /* Border of revealed cell */
#define MINE_CELL_FLAGGED       UI_COLOR_BLACK    /* Flagged cell = black */
#define MINE_CELL_FLAGGED_HI    UI_COLOR_WHITE
#define MINE_CELL_FLAGGED_BD    UI_COLOR_WHITE
#define MINE_CELL_MINE_HIT      UI_COLOR_BLACK    /* Hit mine = black */
#define MINE_CELL_MINE          UI_COLOR_BLACK     /* Revealed mine = black */

/* Number colors (1-8) - all black on white background */
static const ui_color_t s_num_colors[8] = {
    UI_COLOR_BLACK,  /* 1 */
    UI_COLOR_BLACK,  /* 2 */
    UI_COLOR_BLACK,  /* 3 */
    UI_COLOR_BLACK,  /* 4 */
    UI_COLOR_BLACK,  /* 5 - Orange */
    UI_COLOR_BLACK,  /* 6 - Teal */
    UI_COLOR_BLACK,  /* 7 - Black */
    UI_COLOR_BLACK,  /* 8 - Gray */
};
...
static void mine_draw_cell_revealed(int row, int col)
{
    ...
    if (s_mine.flags[row][col] & CELL_MINE) {
        ...
        if (num > 0 && num <= 8) {
            char buf[2] = {'0' + num, '\0'};
            ui_draw_text_in_rect(&r, buf, UI_FONT_TITLE,
...
static void mine_draw_panel(void)
{
    ...
    ui_draw_text_in_rect(&ml, "MINES", UI_FONT_BODY, UI_COLOR_TEXT_SECONDARY, 1);

    /* Mines box */
    ui_rect_t mb = {MINE_PANEL_X, MINE_MINES_BOX_Y, MINE_PANEL_W, MINE_INFO_BOX_H};
    ui_draw_fill_round_rect(&mb, MINE_INFO_BOX_R, UI_COLOR_BLACK);
    ui_draw_text_in_rect(&mb, s_mine.buf_mines, UI_FONT_TITLE, UI_COLOR_WHITE, 1);

    /* Time label */
    ui_rect_t tl = {MINE_PANEL_X, MINE_TIME_LABEL_Y, MINE_PANEL_W, MINE_INFO_LABEL_H};
    ui_draw_text_in_rect(&tl, "TIME", UI_FONT_BODY, UI_COLOR_TEXT_SECONDARY, 1);
    ...
    ui_draw_text_in_rect(&tb, s_mine.buf_time, UI_FONT_TITLE, UI_COLOR_WHITE, 1);

    /* Mode label */
    ui_rect_t mol = {MINE_PANEL_X, MINE_MODE_LABEL_Y, MINE_PANEL_W, MINE_INFO_LABEL_H};
    ui_draw_text_in_rect(&mol, "MODE", UI_FONT_BODY, UI_COLOR_TEXT_SECONDARY, 1);

    /* Mode box */
    ui_rect_t mob = {MINE_PANEL_X, MINE_MODE_BOX_Y, MINE_PANEL_W, MINE_INFO_BOX_H};
    ui_draw_fill_round_rect(&mob, MINE_INFO_BOX_R, UI_COLOR_BLACK);
    const char *mode_str = (s_mine.mode == MINE_MODE_EASY) ? "9x9" : "16x16";
    ui_draw_text_in_rect(&mob, mode_str, UI_FONT_TITLE, UI_COLOR_WHITE, 1);
...
static void mine_draw_mode_btns(void)
{
    ...
    ui_draw_text_in_rect(&er, "9x9 Easy", UI_FONT_BODY,
    ...
    ui_draw_text_in_rect(&hr, "16x16 Hard", UI_FONT_BODY,
                         !easy_act ? UI_COLOR_WHITE : UI_COLOR_BLACK, 1);
}
...
static void mine_draw_restart_btn(void)
{
    ...
    ui_draw_text_in_rect(&r, "RESTART", UI_FONT_TITLE,
                         UI_COLOR_WHITE, 1);
}
...
static void mine_draw_tool_btn(void)
{
    ...
    ui_draw_text_in_rect(&tl, "TOOL", UI_FONT_BODY, UI_COLOR_TEXT_SECONDARY, 1);
    ...
    ui_draw_text_in_rect(&tb, (s_mine.tool == TOOL_DIG) ? "DIG" : "FLAG",
                         UI_FONT_TITLE, UI_COLOR_WHITE, 1);
    ...
    if (s_mine.tool == TOOL_FLAG) {
        ui_draw_fill_round_rect(&r, 6, UI_COLOR_BLACK);
        ui_draw_text_in_rect(&r, "FLAG MODE", UI_FONT_TITLE, UI_COLOR_WHITE, 1);
    } else {
        ui_draw_round_rect(&r, 6, UI_COLOR_WHITE, UI_COLOR_BLACK, 1);
        ui_draw_text_in_rect(&r, "DIG MODE", UI_FONT_TITLE, UI_COLOR_BLACK, 1);
    }
}
...
static void mine_draw_idle_overlay(void)
{
    ...
    ui_draw_text_in_rect(&title, "MINESWEEPER", UI_FONT_TITLE, UI_COLOR_BLACK, 1);

    ui_rect_t sub = {gx, gy + gh / 2 + 5, gw, 24};
    ui_draw_text_in_rect(&sub, "Select mode to start", UI_FONT_BODY,
                         UI_COLOR_BLACK, 1);
}
...
static void mine_draw_win_overlay(void)
{
    ...
    ui_rect_t title = {gx, gy + gh / 2 - 30, gw, 30};
    ui_draw_text_in_rect(&title, "YOU WIN!", UI_FONT_TITLE,
                         UI_COLOR_BLACK, 1);
    ...
    ui_draw_text_in_rect(&sr, buf, UI_FONT_BODY, UI_COLOR_BLACK, 1);

    ui_rect_t hr = {gx, gy + gh / 2 + 35, gw, 24};
    ui_draw_text_in_rect(&hr, "Press RESTART", UI_FONT_BODY,
                         UI_COLOR_BLACK, 1);
}
...
static void mine_draw_lose_overlay(void)
{
    ...
    ui_rect_t title = {gx, gy + gh / 2 - 30, gw, 30};
    ui_draw_text_in_rect(&title, "GAME OVER", UI_FONT_TITLE,
                         UI_COLOR_BLACK, 1);
...
static void mine_game_update(ui_page_t *page, uint32_t dt_ms)
{
    (void)page;
